    ...
    <div class="main-content">
      ...
      <div v-if="environmentData" class="env-status-grid">
        ...
        <div class="browser-cards">
          <div
            v-for="browser in environmentData.system_browsers"
            :key="browser.name"
            class="browser-card"
          >
            <div class="browser-content">
              ...
              <div class="browser-info">
                ...
                <div class="status-row">
                  <el-tag
                    :type="browser.installed ? 'success' : 'info'"
                    effect="dark"
                  >
                    {{
                      browser.installed
                        ? browser.version || $t("configuration.uiEnv.installed")
                        : $t("configuration.uiEnv.notInstalled")
                    }}
                  </el-tag>
                </div>
              </div>
        ...
        <div class="browser-cards">
          <div
            v-for="browser in environmentData.playwright_browsers"
            :key="browser.name"
            class="browser-card"
          >
            <div class="browser-content">
              ...
              <div class="browser-info">
                ...
                <div class="status-row">
                  <el-tag
                    :type="browser.installed ? 'success' : 'warning'"
                    effect="dark"
                  >
                    {{
                      browser.installed
...
const getBrowserIcon = (name) => {
  const iconMap = {
    ...
    chromium:
      "https://raw.githubusercontent.com/alrra/browser-logos/main/src/chromium/chromium_48x48.png",
    webkit:
      "https://raw.githubusercontent.com/alrra/browser-logos/main/src/webkit/webkit_48x48.png",
  };
  return iconMap[name] || "";
};
...
const checkEnvironment = async () => {
  ...
  try {
    const response = await api.get(
      "/ui-automation/config/environment/check_environment/",
    );
...
const installDriver = async (browserName) => {
  ...
  try {
    await api.post("/ui-automation/config/environment/install_driver/", {
      browser: browserName,
    });
    ElMessage.success(
      t("configuration.uiEnv.messages.installSuccess", {
        browser: formatBrowserName(browserName),
      }),
    );
    ...
    ElMessage.error(
      `${t("configuration.uiEnv.messages.installFailed")}: ${error.response?.data?.error || error.message}`,
    );
  } finally {
