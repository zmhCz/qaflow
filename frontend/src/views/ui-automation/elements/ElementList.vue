<template>
  <div class="page-container">
    <div class="page-header" style="display: flex; align-items: center">
      <h1 class="page-title" style="margin-right: 20px; margin-bottom: 0">
        {{ $t("uiAutomation.element.title") }}
      </h1>
      <el-select
        v-model="projectId"
    ...
    <div class="card-container">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="4">
            <el-select
              ...
              :placeholder="$t('uiAutomation.element.strategyFilter')"
              clearable
              @change="handleFilter"
            >
              <el-option
          ...
          <el-col :span="4">
            <el-select
              v-model="pageFilter"
              :placeholder="$t('uiAutomation.element.pageFilter')"
              clearable
              @change="handleFilter"
            >
              <el-option
      ...
      <el-table :data="elements" v-loading="loading" style="width: 100%">
        ...
        <el-table-column
          prop="name"
          :label="$t('uiAutomation.element.elementName')"
          min-width="150"
        >
        ...
        <el-table-column
          prop="page"
          :label="$t('uiAutomation.element.page')"
          width="120"
        />
        <el-table-column
          prop="description"
        ...
        <el-table-column
          prop="locator_value"
          :label="$t('uiAutomation.element.locatorValue')"
          min-width="200"
          show-overflow-tooltip
        />
        <el-table-column
        ...
        >
          <template #default="{ row }">
            <el-button
              size="small"
              type="primary"
              @click="showElementDetail(row.id)"
            >
    ...
    <el-dialog
      ...
      :title="$t('uiAutomation.element.createElement')"
      width="600px"
      ...
      >
        ...
        >
          <el-input
            ...
            :placeholder="$t('uiAutomation.element.rules.nameRequired')"
          />
        </el-form-item>
        <el-form-item :label="$t('uiAutomation.element.page')" prop="page">
          <el-input
            v-model="createForm.page"
            :placeholder="$t('uiAutomation.element.rules.pageRequired')"
          />
        </el-form-item>
        <el-form-item
          :label="$t('uiAutomation.common.description')"
        ...
        >
          <el-input
            v-model="createForm.locator_value"
            :placeholder="$t('uiAutomation.element.rules.locatorRequired')"
          />
        ...
        <el-form-item
          :label="$t('uiAutomation.element.isUnique')"
          prop="is_unique"
        >
          <el-switch v-model="createForm.is_unique" />
        </el-form-item>
        <el-form-item
          :label="$t('uiAutomation.element.waitTimeout')"
          prop="wait_timeout"
        >
          <el-input-number
            v-model="createForm.wait_timeout"
            :min="0"
            :max="30"
            :step="1"
          />
    ...
    <el-dialog
      ...
      :title="$t('uiAutomation.element.editElement')"
      width="600px"
      :close-on-click-modal="false"
    >
      <el-form
      ...
      >
        ...
        >
          <el-input
            v-model="editForm.locator_value"
            :placeholder="$t('uiAutomation.element.rules.locatorRequired')"
          />
          <div class="el-form-item__help">
            <small style="color: #606266">
    ...
    <el-dialog
      ...
      :title="$t('uiAutomation.element.elementDetail')"
      width="600px"
    >
      <div
        v-if="Object.keys(currentElementDetail).length > 0"
        class="element-detail"
      >
        <el-descriptions border :column="{ xs: 1, sm: 2 }">
          <el-descriptions-item
            :label="$t('uiAutomation.element.elementName')"
            >{{ currentElementDetail.name }}</el-descriptions-item
          >
          <el-descriptions-item :label="$t('uiAutomation.element.page')">{{
            currentElementDetail.page
          }}</el-descriptions-item>
          <el-descriptions-item :label="$t('uiAutomation.element.project')">{{
            currentElementDetail.project?.name || "-"
          }}</el-descriptions-item>
          <el-descriptions-item
            :label="$t('uiAutomation.element.locatorStrategy')"
            >{{
              currentElementDetail.locator_strategy?.name || "-"
            }}</el-descriptions-item
          >
          <el-descriptions-item
            :label="$t('uiAutomation.element.locatorValue')"
          ...
          >
            <el-tag
              type="info"
              style="word-break: break-all; display: block; text-align: left"
            >
          ...
          <el-descriptions-item :label="$t('uiAutomation.element.isUnique')">
            <el-tag
              :type="currentElementDetail.is_unique ? 'success' : 'warning'"
            >
              {{
                currentElementDetail.is_unique
                  ? $t("uiAutomation.common.yes")
                  : $t("uiAutomation.common.no")
              }}
            </el-tag>
          </el-descriptions-item>
          <el-descriptions-item :label="$t('uiAutomation.element.waitTimeout')"
            >{{ currentElementDetail.wait_timeout || 5
            }}{{
          ...
          <el-descriptions-item
            ...
            :span="2"
            >{{
              currentElementDetail.description === undefined
                ? "-"
                : currentElementDetail.description
            }}</el-descriptions-item
          >
          <el-descriptions-item :label="$t('uiAutomation.common.createTime')">{{
            formatDate(null, null, currentElementDetail.created_at)
          }}</el-descriptions-item>
          <el-descriptions-item :label="$t('uiAutomation.common.updateTime')">{{
            formatDate(null, null, currentElementDetail.updated_at)
          }}</el-descriptions-item>
          <el-descriptions-item
            :label="$t('uiAutomation.element.createdBy')"
            :span="2"
            >{{
              currentElementDetail.created_by?.username || "-"
            }}</el-descriptions-item
          >
        </el-descriptions>
      </div>
...
const formRules = computed(() => ({
  name: [
    {
      required: true,
      message: t("uiAutomation.element.rules.nameRequired"),
      trigger: "blur",
    },
    {
      min: 2,
      max: 100,
      message: t("uiAutomation.element.rules.nameLength"),
      trigger: "blur",
    },
  ],
  page: [
    {
      required: true,
      message: t("uiAutomation.element.rules.pageRequired"),
      trigger: "blur",
    },
  ],
  strategy: [
    {
      required: true,
      message: t("uiAutomation.element.rules.strategyRequired"),
      trigger: "change",
    },
  ],
  locator_value: [
    {
      required: true,
      message: t("uiAutomation.element.rules.locatorRequired"),
      trigger: "blur",
    },
  ],
}));
...
const editElement = (element) => {
  ...
  const strategy = strategies.value.find(
    (s) => s.name === element.locator_strategy?.name,
  );
...
const handleDeleteElement = async (id) => {
  try {
    await ElMessageBox.confirm(
      t("uiAutomation.element.messages.deleteConfirm"),
  ...
  try {
    ...
    if (error.response && error.response.data) {
      ...
      if (errorData.locator_strategy_id) {
        ...
        ElMessage.error(
          t("uiAutomation.element.messages.createFailed") +
            ": " +
