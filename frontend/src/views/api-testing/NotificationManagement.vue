<template>
  <div class="notification-management">
    ...
    <el-tabs v-model="activeTab" class="notification-tabs">
      <!-- 通知列表Tab -->
      <el-tab-pane
        :label="$t('apiTesting.notification.notificationList')"
        name="list"
      >
        <div class="tab-content">
          ...
          >
            <el-table-column
              prop="task_name"
              :label="$t('apiTesting.notification.taskName')"
              min-width="120"
            />
            <el-table-column
              prop="notify_time"
              :label="$t('apiTesting.notification.notifyTime')"
              min-width="140"
            >
            ...
            <el-table-column
              prop="recipients"
              :label="$t('apiTesting.notification.recipients')"
              min-width="120"
            >
              <template #default="{ row }">
      ...
      <!-- 通知配置Tab -->
      <el-tab-pane
        :label="$t('apiTesting.notification.notificationConfig')"
        name="config"
      >
        <div class="tab-content">
          <!-- 邮箱配置 -->
            ...
            <el-card>
              <div class="config-form">
                <el-form :model="emailConfig" label-width="120px">
                  <el-form-item
                    :label="$t('apiTesting.notification.senderEmail')"
                    required
                  >
                    <el-input
                      v-model="emailConfig.sender_email"
                      :placeholder="
                        $t('apiTesting.notification.inputSenderEmail')
                      "
                    />
                  </el-form-item>
                  <el-form-item
                    :label="$t('apiTesting.notification.smtpServer')"
                    required
                  >
                    <el-input
                      v-model="emailConfig.smtp_host"
                      :placeholder="
                        $t('apiTesting.notification.smtpServerPlaceholder')
                      "
                    />
                  </el-form-item>
                  <el-form-item
                    :label="$t('apiTesting.notification.smtpPort')"
                    required
                  >
                    <el-input-number
                  ...
                  <el-form-item
                    :label="$t('apiTesting.notification.authCode')"
                    required
                  >
                    <el-input
          ...
          <div class="config-section">
            ...
            <el-card>
              ...
              <el-table :data="recipients" style="width: 100%">
                <el-table-column
                  prop="name"
                  :label="$t('apiTesting.notification.recipientName')"
                  width="120"
                />
                <el-table-column
                  prop="email"
                  :label="$t('apiTesting.notification.emailAddress')"
                  min-width="200"
                />
                <el-table-column
                  :label="$t('apiTesting.common.operation')"
          ...
          <div class="config-section">
            ...
            <el-card>
              ...
              <el-table :data="webhooks" style="width: 100%">
                <el-table-column
                  prop="name"
                  :label="$t('apiTesting.common.name')"
                  width="120"
                />
                <el-table-column
                  prop="platform"
                  :label="$t('apiTesting.notification.platform')"
                  width="100"
                >
                  <template #default="{ row }">
                ...
                <el-table-column
                  prop="webhook_url"
                  :label="$t('apiTesting.notification.webhookAddress')"
                  min-width="200"
                />
                <el-table-column
                  prop="enabled"
    ...
    >
      <div v-if="currentNotification" class="notification-detail">
        <el-descriptions :column="1" border>
          ...
          <el-descriptions-item
            :label="$t('apiTesting.notification.notificationType')"
          >
            {{
              currentNotification.notify_type === "EMAIL"
                ? $t("apiTesting.notification.emailNotification")
                : $t("apiTesting.notification.webhookNotification")
            }}
          </el-descriptions-item>
          <el-descriptions-item
            :label="$t('apiTesting.notification.recipients')"
          ...
          <el-descriptions-item :label="$t('apiTesting.common.status')">
            <el-tag
              :type="
                currentNotification.status === 'SUCCESS' ? 'success' : 'danger'
              "
            >
              {{
          ...
          <el-descriptions-item
            ...
            v-if="currentNotification.error_message"
          >
      ...
      <el-form :model="newRecipient" label-width="80px">
        ...
        >
          <el-input
            ...
            :placeholder="$t('apiTesting.notification.inputName')"
          />
        </el-form-item>
        <el-form-item
          :label="$t('apiTesting.notification.emailAddress')"
        ...
        >
          <el-input
            ...
            :placeholder="$t('apiTesting.notification.inputEmail')"
          />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="showRecipientDialog = false">{{
          $t("apiTesting.common.cancel")
        }}</el-button>
        <el-button type="primary" @click="addRecipient">{{
      ...
      <el-form :model="newWebhook" label-width="100px">
        <el-form-item :label="$t('apiTesting.common.name')" required>
          <el-input
            ...
            :placeholder="$t('apiTesting.notification.inputWebhookName')"
          />
        </el-form-item>
        <el-form-item :label="$t('apiTesting.notification.platform')" required>
          <el-select
            v-model="newWebhook.platform"
            :placeholder="$t('apiTesting.notification.selectPlatform')"
          >
            <el-option
              :label="$t('apiTesting.notification.platforms.feishu')"
              value="FEISHU"
            />
            <el-option
              :label="$t('apiTesting.notification.platforms.wechatWork')"
              value="WECHAT_WORK"
            />
            <el-option
              :label="$t('apiTesting.notification.platforms.dingtalk')"
              value="DINGTALK"
            />
          </el-select>
        </el-form-item>
...
const deleteWebhook = async (webhook) => {
  try {
    ...
    const index = webhooks.value.findIndex(
      (w) => w.webhook_url === webhook.webhook_url,
    );
    if (index !== -1) {
...
const toggleWebhookStatus = async (webhook) => {
  try {
    ElMessage.success(
      webhook.enabled
        ? t("apiTesting.notification.webhookEnabled")
        : t("apiTesting.notification.webhookDisabled"),
    );
  } catch (error) {
...
const getPlatformName = (platform) => {
  ...
  return platformKey
    ? t(`apiTesting.notification.platforms.${platformKey}`)
    : platform;
};
