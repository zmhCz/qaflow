  ...
  <div class="script-list">
    ...
    <div class="main-content">
      <el-table :data="scripts" stripe style="width: 100%">
        <el-table-column
          type="index"
          :label="$t('uiAutomation.script.index')"
          width="60"
        />
        <el-table-column
          :label="$t('uiAutomation.script.projectColumn')"
          width="150"
        >
        ...
        <el-table-column
          prop="name"
          :label="$t('uiAutomation.script.nameColumn')"
          min-width="300"
          show-overflow-tooltip
        />
        <el-table-column
          :label="$t('uiAutomation.script.languageColumn')"
          width="100"
        >
          <template #default="{ row }">
            <el-tag
              size="small"
              :type="row.language === 'python' ? 'success' : 'primary'"
            >
        ...
        <el-table-column
          :label="$t('uiAutomation.script.frameworkColumn')"
          width="120"
        >
          <template #default="{ row }">
            <el-tag
              size="small"
              :type="row.framework === 'playwright' ? 'warning' : 'info'"
            >
        ...
        <el-table-column
          prop="created_at"
          :label="$t('uiAutomation.script.createTimeColumn')"
          width="180"
        >
          <template #default="{ row }">
    ...
    <el-dialog
      ...
      :title="$t('uiAutomation.script.scriptDetail')"
      width="70%"
    >
      <div v-if="currentScript" class="script-detail">
        <el-descriptions :column="2" border>
          <el-descriptions-item
            :label="$t('uiAutomation.script.scriptName')"
            :span="2"
            >{{ currentScript.name }}</el-descriptions-item
          >
          <el-descriptions-item :label="$t('uiAutomation.script.project')">{{
            currentScript.project?.name ||
            $t("uiAutomation.script.unknownProject")
          }}</el-descriptions-item>
          <el-descriptions-item :label="$t('uiAutomation.script.language')">{{
            getLanguageText(currentScript.language)
          }}</el-descriptions-item>
          <el-descriptions-item :label="$t('uiAutomation.script.framework')">{{
            getFrameworkText(currentScript.framework)
          }}</el-descriptions-item>
          <el-descriptions-item :label="$t('uiAutomation.script.scriptType')">{{
            getScriptTypeText(currentScript.script_type)
          }}</el-descriptions-item>
          <el-descriptions-item
            :label="$t('uiAutomation.script.createTime')"
            :span="2"
            >{{ formatTime(currentScript.created_at) }}</el-descriptions-item
          >
          <el-descriptions-item
            :label="$t('uiAutomation.script.updateTime')"
            :span="2"
            >{{ formatTime(currentScript.updated_at) }}</el-descriptions-item
          >
        </el-descriptions>
        ...
        <div class="script-content">
          ...
          <pre class="code-view">{{
            currentScript.content || $t("uiAutomation.script.noContent")
          }}</pre>
        </div>
      </div>
      <template #footer>
        <el-button @click="showDetailDialog = false">{{
          $t("uiAutomation.script.close")
        }}</el-button>
        <el-button type="primary" @click="editScript(currentScript)">{{
          $t("uiAutomation.script.editScript")
        }}</el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog
      ...
      :title="$t('uiAutomation.script.renameScript')"
      width="400px"
    >
    ...
    <el-dialog
      ...
      :title="$t('uiAutomation.script.editScript')"
      width="80%"
    ...
    >
        ...
        <div class="editor-header">
          ...
          <div class="editor-info">
            <el-tag
              size="small"
              :type="
                editingScript.language === 'python' ? 'success' : 'primary'
              "
            >
            ...
            <el-tag
              size="small"
              :type="
                editingScript.framework === 'playwright' ? 'warning' : 'info'
              "
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="saveEditedScript" :loading="saving">{{
          $t("uiAutomation.script.save")
        }}</el-button>
      </template>
    </el-dialog>
  </div>
...
const getFrameworkText = (framework) => {
  ...
  return (
    frameworkMap[framework] || framework || t("uiAutomation.status.unknown")
  );
};
