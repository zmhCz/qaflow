          ...
          <div class="detail-header">
            ...
            <div class="detail-actions">
              ...
              <el-select
                ...
                :placeholder="t('uiAutomation.testCase.runModeLabel')"
                size="small"
                ...
                >
                  <div class="screenshots-container">
                    <div
                      ...
                      @click="previewScreenshot(screenshot, index)"
                    >
    ...
    >
      <div v-if="currentScreenshot" class="screenshot-preview">
        <div class="preview-info">
          ...
          <p>{{ currentScreenshotIndex + 1 }} / {{ screenshotPreviewTotal }}</p>
        ...
        <div class="preview-image">
          <el-button
            ...
            @click="switchScreenshotPreview(-1)"
          >
            上一张
          </el-button>
          <img
          ...
          <el-button
            class="preview-nav-btn"
            :disabled="currentScreenshotIndex >= screenshotPreviewTotal - 1"
            @click="switchScreenshotPreview(1)"
          >
            下一张
          </el-button>
...
const screenshotPreviewTotal = computed(
  () => executionResult.value?.screenshots?.length || 0,
);
...
const previewScreenshot = (screenshot, index = 0) => {
  currentScreenshot.value = screenshot;
  currentScreenshotIndex.value = Number.isFinite(Number(index))
    ? Number(index)
    : 0;
  showScreenshotPreview.value = true;
};

const switchScreenshotPreview = (direction) => {
  const screenshots = executionResult.value?.screenshots || [];
  if (!screenshots.length) return;
  const nextIndex = currentScreenshotIndex.value + direction;
  if (nextIndex < 0 || nextIndex >= screenshots.length) return;
  currentScreenshotIndex.value = nextIndex;
  currentScreenshot.value = screenshots[nextIndex];
};
