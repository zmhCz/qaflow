  ...
  <div class="page-container">
    <div class="page-header">
      ...
      <div>
        ...
        <el-button v-if="canEdit" type="warning" @click="editReview">{{
          $t("reviewDetail.edit")
        }}</el-button>
        <el-button v-if="canReview" type="success" @click="showReviewDialog">{{
          $t("reviewDetail.submitReview")
        }}</el-button>
      </div>
    </div>
    ...
    <div v-if="review" class="content-container">
      ...
      <el-card class="info-card">
        ...
        <el-descriptions :column="2" border>
          <el-descriptions-item
            :label="$t('reviewDetail.reviewTitle')"
            :span="2"
            >{{ review.title }}</el-descriptions-item
          >
          ...
          <el-descriptions-item :label="$t('reviewDetail.creator')">{{
          ...
          <el-descriptions-item :label="$t('reviewDetail.priority')">
            <el-tag :class="`priority-tag ${review.priority}`">{{
          ...
          <el-descriptions-item
            ...
            :span="2"
            >{{
              review.description || $t("reviewDetail.none")
            }}</el-descriptions-item
          >
        </el-descriptions>
      </el-card>
      ...
      <el-card class="reviewers-card">
        <template #header>
          <span class="card-header">{{
            $t("reviewDetail.reviewersCard")
          }}</span>
        </template>
        <el-table :data="review.assignments" stripe>
          <el-table-column
            prop="reviewer.username"
            :label="$t('reviewDetail.reviewer')"
            width="150"
          />
          <el-table-column
            :label="$t('reviewDetail.reviewerStatus')"
            width="120"
          >
          ...
          <el-table-column
            prop="comment"
            :label="$t('reviewDetail.reviewerComment')"
            min-width="200"
            show-overflow-tooltip
          />
          <el-table-column
            v-if="review.template?.checklist?.length"
            :label="$t('reviewDetail.checklist')"
            width="150"
          >
            <template #default="{ row }">
              <div
                v-if="
                  ...
                  Object.keys(row.checklist_results).length > 0
                "
                ...
                <el-tooltip effect="dark" placement="top">
                  <template #content>
                    <div class="checklist-tooltip">
                      ...
                      >
                        <el-icon
                          v-if="row.checklist_results[index] === true"
                          class="pass-icon"
                          ><Check
                        /></el-icon>
                        <el-icon
                          v-else-if="row.checklist_results[index] === false"
                          class="fail-icon"
                          ><Close
                        /></el-icon>
              ...
              <span v-else class="no-checklist">{{
                $t("reviewDetail.notFilled")
              }}</span>
            </template>
          </el-table-column>
          <el-table-column
            prop="assigned_at"
            :label="$t('reviewDetail.assignedAt')"
            width="160"
          >
            <template #default="{ row }">
          ...
          <el-table-column
            prop="reviewed_at"
            :label="$t('reviewDetail.reviewedAt')"
            width="160"
          >
            <template #default="{ row }">
              {{
      ...
      <el-card class="testcases-card">
        <template #header>
          <span class="card-header"
            >{{ $t("reviewDetail.reviewTestcases") }} ({{
        ...
        <el-table :data="review.testcases" stripe>
          <el-table-column
            prop="title"
            :label="$t('reviewDetail.testcaseTitle')"
            min-width="200"
            show-overflow-tooltip
          />
          ...
          <el-table-column
            ...
            :label="$t('reviewDetail.author')"
            width="120"
          />
          <el-table-column :label="$t('reviewList.actions')" width="120">
            <template #default="{ row }">
              <el-button link type="primary" @click="viewTestcase(row.id)">{{
                $t("reviewDetail.view")
              }}</el-button>
              <el-button link type="success" @click="addComment(row)">{{
                $t("reviewDetail.comment")
              }}</el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <el-card class="comments-card">
        <template #header>
          <div class="card-header-with-action">
            <span class="card-header">{{
              $t("reviewDetail.reviewComments")
            }}</span>
            <el-button
              type="primary"
              size="small"
              @click="showAddCommentDialog"
              >{{ $t("reviewDetail.addComment") }}</el-button
            >
          </div>
        </template>
          ...
          >
            <div class="comment-header">
              <div class="comment-author">
                ...
                <el-tag
                  size="small"
                  :type="getCommentTypeColor(comment.comment_type)"
                >
            ...
            <div v-if="comment.testcase" class="comment-testcase">
              {{ $t("reviewDetail.relatedTestcase") }}:
    ...
    <el-dialog
      ...
      :title="$t('reviewDetail.submitReviewDialog')"
    ...
    >
      <el-form :model="reviewForm" label-width="100px">
        <el-form-item :label="$t('reviewDetail.reviewResult')" required>
          <el-radio-group v-model="reviewForm.status">
            <el-radio-button label="approved">{{
              $t("reviewDetail.approved")
            }}</el-radio-button>
            <el-radio-button label="rejected">{{
              $t("reviewDetail.rejected")
            }}</el-radio-button>
            <el-radio-button label="abstained">{{
              $t("reviewDetail.abstained")
            }}</el-radio-button>
          </el-radio-group>
        </el-form-item>
        ...
        <el-form-item
          v-if="review?.template?.checklist?.length"
          :label="$t('reviewDetail.checklist')"
          class="checklist-form-item"
        >
          <div class="checklist-container">
            <div class="checklist-header">
              <span class="checklist-title"
                >{{ review.template.name }} -
                {{ $t("reviewDetail.checklistTitle") }}</span
              >
              <div class="checklist-actions">
                <el-button size="small" @click="checkAll(true)">{{
                  $t("reviewDetail.allPass")
                }}</el-button>
                <el-button size="small" @click="checkAll(false)">{{
                  $t("reviewDetail.allFail")
                }}</el-button>
              </div>
            </div>
    ...
    <el-dialog
      ...
      :title="$t('reviewDetail.addCommentDialog')"
      :close-on-click-modal="false"
      width="600px"
    >
        ...
        <el-form-item :label="$t('reviewDetail.commentType')" required>
          <el-radio-group v-model="commentForm.comment_type">
            <el-radio-button label="general">{{
              $t("reviewDetail.generalComment")
            }}</el-radio-button>
            <el-radio-button label="testcase">{{
              $t("reviewDetail.testcaseComment")
            }}</el-radio-button>
          </el-radio-group>
        </el-form-item>
        <el-form-item
          v-if="commentForm.comment_type === 'testcase'"
          :label="$t('reviewDetail.relatedTestcaseLabel')"
        >
          <el-select
            v-model="commentForm.testcase"
            :placeholder="$t('reviewDetail.selectTestcase')"
          >
            <el-option
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="addCommentSubmit">{{
          $t("reviewDetail.submit")
        }}</el-button>
      </template>
    </el-dialog>
...
const completedReviews = computed(() => {
  return (
    review.value?.assignments?.filter((a) => a.status !== "pending").length || 0
  );
});
...
const canEdit = computed(() => {
  return (
    review.value?.creator?.id === userStore.user?.id &&
    ["pending", "in_progress"].includes(review.value?.status)
  );
});

const canReview = computed(() => {
  return review.value?.assignments?.some(
...
const submitReview = async () => {
  try {
    await api.post(
      `/reviews/reviews/${route.params.id}/submit_review/`,
      reviewForm,
    );
...
const getChecklistStats = (checklistResults, checklist) => {
  ...
  const passed = Object.values(checklistResults).filter(
    (result) => result === true,
  ).length;
  const failed = Object.values(checklistResults).filter(
    (result) => result === false,
  ).length;
