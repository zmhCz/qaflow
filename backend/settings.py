...
# 单机真机调试时 Redis/Celery 容易因重启遗漏启动。开发环境默认允许本地后台线程兜底，
# 避免 APP 自动化执行记录长期卡在“等待中”；生产环境可显式设为 false。
APP_AUTOMATION_LOCAL_TASK_FALLBACK = config(
    'APP_AUTOMATION_LOCAL_TASK_FALLBACK',
    default=DEBUG,
    cast=bool,
)
...
# 数据工厂业务压测默认配置
# 生产/演示环境请通过 .env 注入真实业务地址、验证码和外部压测工具路径，仓库内不保存公司环境信息。
DATA_FACTORY_DEFAULT_BASE_URL = config('DATA_FACTORY_DEFAULT_BASE_URL', default='')
DATA_FACTORY_DEFAULT_SMS_CODE = config('DATA_FACTORY_DEFAULT_SMS_CODE', default='')
DATA_FACTORY_DEFAULT_PROBE_PHONE = config('DATA_FACTORY_DEFAULT_PROBE_PHONE', default='')
DATA_FACTORY_IM_RUNNER_PATH = config('DATA_FACTORY_IM_RUNNER_PATH', default='')

# APP 自动化样例/种子用例默认测试数据
APP_AUTOMATION_TEST_LOGIN_PHONE = config('APP_AUTOMATION_TEST_LOGIN_PHONE', default='')
APP_AUTOMATION_TEST_LOGIN_PASSWORD = config('APP_AUTOMATION_TEST_LOGIN_PASSWORD', default='')
APP_AUTOMATION_TEST_COMMUNITY_KEYWORD = config('APP_AUTOMATION_TEST_COMMUNITY_KEYWORD', default='')
APP_AUTOMATION_TEST_COMMUNITY_NAME = config('APP_AUTOMATION_TEST_COMMUNITY_NAME', default='')
APP_AUTOMATION_TARGET_PACKAGE = config('APP_AUTOMATION_TARGET_PACKAGE', default='')
APP_AUTOMATION_MAIN_ACTIVITY = config('APP_AUTOMATION_MAIN_ACTIVITY', default='')
APP_AUTOMATION_SEARCH_ROOM_ACTIVITY = config('APP_AUTOMATION_SEARCH_ROOM_ACTIVITY', default='')
APP_AUTOMATION_PUBLISH_TEAM_ACTIVITY = config('APP_AUTOMATION_PUBLISH_TEAM_ACTIVITY', default='')
APP_AUTOMATION_ROOM_ACTIVITY = config('APP_AUTOMATION_ROOM_ACTIVITY', default='')
