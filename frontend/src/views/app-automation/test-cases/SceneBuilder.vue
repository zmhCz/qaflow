          ...
          <el-tabs v-model="paletteTab" stretch>
            <el-tab-pane label="基础组件" name="base">
              ...
              >
                ...
                <template #footer>
                  <div
                    v-if="componentPalette.length === 0"
                    class="palette-empty"
                  >
                    暂无基础组件
                  </div>
            ...
            <el-tab-pane label="自定义组件" name="custom">
              ...
              >
                  ...
                  <div class="palette-item">
                    ...
                    <div class="palette-actions">
                      <el-button
                        link
                        size="small"
                        @click.stop="openEditCustomComponent(element)"
                      >
                        编辑
                      </el-button>
                      <el-button
                ...
                <template #footer>
                  <div
                    v-if="customComponentPalette.length === 0"
                    class="palette-empty"
                  >
                    暂无自定义组件
                  </div>
          ...
          >
            <template #item="{ element, index }">
              <div class="scene-item-wrapper">
                ...
                <!-- 自定义组件展开的子步骤 -->
                <div
                  v-if="element._expanded && element.steps"
                  class="custom-sub-steps"
                >
                  <div
                  ...
                  >
                    <div class="scene-item-main">
                      <span class="scene-index sub-index"
                        >{{ Number(index) + 1 }}.{{ Number(subIdx) + 1 }}</span
                      >
                      <span class="scene-name">{{
                        subStep.name || subStep.type
                      }}</span>
          ...
          <div v-else class="config-form">
            <el-form :model="activeStep" label-width="110px" size="small">
              ...
              <div class="variable-hint" v-pre>
                ...
                {{ outputs.last.xxx }} / {{ steps.step_id.xxx }}
              </div>
              <div
                v-if="activeStep && activeStep.type === 'image_exists_click'"
              ...
              <div
                v-if="
                  activeStep && activeStep.type === 'image_exists_click_chain'
                "
                class="variable-hint hint-danger"
              >
              ...
              <div
                v-if="activeStep && activeStep.type === 'foreach_assert'"
                class="variable-hint hint-danger"
              >
                逻辑：先点击，后断言。
              </div>
              <div
                v-if="activeStep && activeStep.type === 'api_request'"
                class="api-template-block"
              >
                <div class="tool-hint">API 示例模板</div>
                <div class="template-list">
                  <div
                    v-for="item in apiRequestTemplates"
                    :key="item.name"
                    class="template-item"
                  >
                  ...
                  <div
                    v-if="apiRequestTemplates.length === 0"
                    class="tool-hint"
                  >
                    暂无模板
                  </div>
              ...
              <!-- 统一的字段分组显示 -->
              <el-form-item
                v-if="activeStep && activeStep.type === 'wait'"
                label="等待时长"
              >
                <el-input-number
                  :model-value="getWaitSeconds(activeStep)"
                  :min="0.1"
                  :max="300"
                  :step="0.1"
                  :precision="1"
                  controls-position="right"
                  @update:model-value="updateWaitSeconds(activeStep, $event)"
                />
              ...
              <template v-if="schemaFields.length > 0">
                ...
                >
                  ...
                  >
                    已关联: {{ linkedElements[group.key].name }} ({{
                      linkedElements[group.key].type
                    }})
                    <el-button
                      link
                      type="danger"
                  ...
                  >
                    <el-input
                      ...
                      :rows="4"
                      :model-value="
                        getExpectedListValue(activeStep && activeStep.config)
                      "
                      :placeholder="getFieldPlaceholder(field)"
                      @update:model-value="
                        updateExpectedList(activeStep.config, $event)
                      "
                    />
                    <el-input
    ...
    >
      <el-form label-width="110px" size="small">
        ...
        <div class="package-list" v-loading="packageLoading">
          ...
          <div v-if="packageList.length === 0" class="package-empty">
            暂无组件包
          </div>
        </div>
      </el-form>
            ...
            <div v-else class="config-form">
              ...
              >
                ...
                <div
                  v-if="
                    editingActiveStep &&
                    editingActiveStep.type === 'api_request'
                  "
                  class="api-template-block"
                >
                  <div class="tool-hint">API 示例模板</div>
                    ...
                    >
                      ...
                      <div class="template-actions">
                        <el-button
                          ...
                          size="small"
                          @click="
                            applyApiRequestTemplate(item, editingActiveStep)
                          "
                        >
                          使用
                        </el-button>
                ...
                <!-- 分组显示（与场景步骤配置面板一致） -->
                <el-form-item
                  v-if="editingActiveStep && editingActiveStep.type === 'wait'"
                  label="等待时长"
                >
                  <el-input-number
                    :model-value="getWaitSeconds(editingActiveStep)"
                    :min="0.1"
                    :max="300"
                    :step="0.1"
                    :precision="1"
                    controls-position="right"
                    @update:model-value="
                      updateWaitSeconds(editingActiveStep, $event)
                    "
                  />
                  <span class="field-unit">秒</span>
                </el-form-item>
                  ...
                  >
                    ...
                    >
                      <el-input
                        ...
                        :rows="4"
                        :model-value="
                          getExpectedListValue(
                            editingActiveStep && editingActiveStep.config,
                          )
                        "
                        :placeholder="getFieldPlaceholder(field)"
                        @update:model-value="
                          updateExpectedList(editingActiveStep.config, $event)
                        "
                      />
                      <el-input
                        v-else-if="isJsonField(field)"
                      ...
                      <el-switch
                        v-else-if="
                          getFieldTypeForDef(editingActiveDef, field) ===
                          'boolean'
                        "
                        v-model="editingActiveStep.config[field]"
                      />
                      <el-input-number
                        ...
                        v-model="editingActiveStep.config[field]"
                        :min="
                          getFieldNumberRuleForDef(editingActiveDef, field).min
                        "
                        :max="
                          getFieldNumberRuleForDef(editingActiveDef, field).max
                        "
                        :step="
                          getFieldNumberRuleForDef(editingActiveDef, field).step
                        "
                        controls-position="right"
                      />
                      <el-input
    ...
    >
      <div class="element-selector-container">
        ...
        <div class="element-selector-filter">
          <el-space wrap>
            <el-radio-group
              v-model="elementFilterType"
              @change="handleElementFilterChange"
            >
            ...
            <el-input
              ...
              placeholder="默认 semantic_v2，可搜索业务名/元素名"
              style="width: 320px"
              clearable
              @clear="handleElementSearch"
              @keyup.enter="handleElementSearch"
            >
              <template #prefix>
              ...
              <template #suffix>
                <el-button
                  v-if="elementSearchKeyword"
                  type="primary"
                  link
                  :icon="Search"
                  @click="handleElementSearch"
                  style="padding: 0"
                />
              </template>
            </el-input>
        ...
        >
          ...
          >
            <template #default="{ row }">
              <span>{{ getElementDisplayName(row) }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column label="图片分类" width="120">
            <template #default="{ row }">
              <el-tag
                v-if="
                  row.element_type === 'image' && row.config?.image_category
                "
                type="info"
                size="small"
              >
            ...
            <template #default="{ row }">
              ...
              >
                <el-space direction="vertical" :size="4" alignment="start">
                  ...
                  <el-tag
                    v-else-if="row.config?.locator_key"
                    type="info"
                    size="small"
                  >
                    key: {{ row.config.locator_key }}
                  </el-tag>
                  <el-tag v-if="row.config?.bounds" type="warning" size="small">
                    bounds: {{ row.config.bounds }}
                  </el-tag>
                </el-space>
              </div>
            </template>
...
const editingSchemaFields = computed(() => {
  ...
  let visible = ordered.filter((field) =>
    shouldShowField(field, editingActiveStep.value),
  );
...
const customDialogTitle = computed(() => {
  return customDialogMode.value === "edit"
    ? "编辑自定义组件"
    : "另存为自定义组件";
});
...
const schemaFields = computed(() => {
  ...
  let visible = ordered.filter((field) =>
    shouldShowField(field, activeStep.value),
  );
  return getFieldOrder(visible);
});
...
watch(
  () => activeStep.value?.config?.assert_type,
  (value) => {
...
// Methods
const getWaitSeconds = (step) => {
  if (!step || !step.config) {
    return 3;
  }
  const value = step.config.duration ?? step.config.timeout ?? 3;
  const seconds = Number(value);
  return Number.isFinite(seconds) && seconds > 0 ? seconds : 3;
};

const updateWaitSeconds = (step, value) => {
  if (!step) {
    return;
  }
  if (!step.config) {
  ...
  const seconds = Number(value);
  const normalized =
    Number.isFinite(seconds) && seconds > 0 ? Math.round(seconds * 10) / 10 : 3;
  step.config.duration = normalized;
  step.config.timeout = normalized;
  step.name = `Wait ${normalized.toFixed(1)} seconds`;
};
...
const shouldShowField = (field, step) => {
  ...
  if (step.type === "wait" && field === "timeout") {
    return false;
  }

  const assertType = step.config.assert_type;
  if (
    (field === "package_name" || field === "activity") &&
    assertType !== "page" &&
    assertType !== "activity"
  ) {
    return false;
  ...
  if (assertType === "page" || assertType === "activity") {
    if (
      [
        "selector_type",
        "selector",
  ...
  if (step.type === "loop") {
    ...
    if (
      (field === "items" || field === "item_var" || field === "item_scope") &&
      mode !== "foreach"
    ) {
      return false;
    }
...
const addSubStep = (parentIndex) => {
  ...
  const newStep = {
    ...
    name: "点击",
    config: {
      ...(componentDefinitions.value["click"]?.default_config || {
        selector_type: "image",
        timeout: 5,
      }),
    },
...
const openCustomComponentDialog = () => {
  ...
  const hasCustomStep = scenarioSteps.value.some(
    (step) => step.kind === "custom",
  );
  if (hasCustomStep) {
...
const openEditCustomComponent = (item) => {
  ...
  const rawSteps =
    item.steps && Array.isArray(item.steps)
      ? item.steps
      : (customComponentDefinitions.value[item.type] &&
          customComponentDefinitions.value[item.type].steps) ||
        [];
  editingCustomSteps.value = JSON.parse(JSON.stringify(rawSteps)).map(
    (step) => ({
...
const addEditStep = () => {
  ...
  const item = componentPalette.value.find(
    (component) => component.type === editStepType.value,
  );
  if (!item) {
...
const saveCustomComponent = () => {
  customFormRef.value.validate(async (valid) => {
    ...
    if (
      customDialogMode.value === "edit" &&
      editingCustomSteps.value.length === 0
    ) {
  ...
  try {
    ...
    if (list.length > 0) {
      const mergedList = list.map((item) => ({
        type: item.type,
        name: item.name,
        schema: normalizeComponentSchema(item.type, item.schema || {}),
        defaultConfig: normalizeComponentDefaultConfig(
          item.type,
          item.default_config || {},
        ),
        raw: item,
      }));
      componentPalette.value = mergedList
        .map((item) => ({
          type: item.type || "",
          name: item.name || "",
          schema: item.schema || {},
      ...
      ElMessage.warning(
        "暂无组件定义，请先初始化组件库（后端运行：python manage.py load_component_pack）",
      );
    }
  } catch (error) {
...
const normalizeComponentSchema = (type, schema) => {
  if (type !== "assert") {
    return schema || {};
  }
  const normalized = JSON.parse(JSON.stringify(schema || {}));
  normalized.properties = normalized.properties || {};
  if (!normalized.properties.package_name) {
    normalized.properties.package_name = { type: "string" };
  }
  if (!normalized.properties.activity) {
    normalized.properties.activity = { type: "string" };
  }
  return normalized;
};

const normalizeComponentDefaultConfig = (type, config) => {
  if (type !== "assert") {
    return config || {};
  }
  return {
...
const loadCustomComponentPalette = async () => {
  defaultCustomPalette.value = JSON.parse(
    JSON.stringify(customComponentPalette.value),
  );
  try {
    ...
    if (list.length > 0) {
      customComponentPalette.value = list
        .map((item) => ({
      ...
      customComponentPalette.value = defaultCustomPalette.value
        .map((item) => ({
          ...item,
          id: item.id || `custom_${item.type}_${Date.now()}_${Math.random()}`,
        }))
        .filter((item) => item.id && item.type);
      customComponentDefinitions.value = customComponentPalette.value.reduce(
        (acc, item) => {
          if (item && item.type) {
            acc[item.type] = item;
          }
...
const loadCaseDetail = async (caseId) => {
  try {
    ...
    if (data && data.id) {
      ...
      scenarioSteps.value = (
        Array.isArray(data.ui_flow) ? data.ui_flow : []
      ).map((step) => {
        // 确保每个步骤有 id
...
const getFieldGroups = (fieldsOverride = null) => {
  ...
  const groupRules = [
    ...
    },
    {
      ...
      hasFields: () =>
        allFields.includes("assert_type") || allFields.includes("expected"),
    },
  ];
...
const getFieldLabel = (field) => {
  ...
  if (
    config &&
    (config.assert_type === "page" || config.assert_type === "activity")
  ) {
    if (field === "expected") {
      return "Activity关键字";
    }
...
const getFieldPlaceholder = (field) => {
  ...
  const getDynamicPlaceholder = (selectorField, typeField) => {
    ...
    if (type === "selector") return "selector: 关联元素 / resource_id";
    return null;
  };

  if (field === "selector") {
    return (
      getDynamicPlaceholder("selector", "selector_type") ||
      "image: 文件名 | pos: x,y | region: x1,y1,x2,y2"
    );
  }
  if (field === "fallback_selector") {
    return (
      getDynamicPlaceholder("fallback_selector", "fallback_selector_type") ||
      "image: 文件名 | pos: x,y | region: x1,y1,x2,y2"
    );
  }
  if (field === "click_selector") {
    return (
      getDynamicPlaceholder("click_selector", "click_selector_type") ||
      "image: 文件名 | pos: x,y"
    );
  }
  if (field === "start_selector") {
    return (
      getDynamicPlaceholder("start_selector", "start_selector_type") ||
      "image: 文件名 | pos: x,y"
    );
  }
  if (field === "end_selector") {
    return (
      getDynamicPlaceholder("end_selector", "end_selector_type") ||
      "image: 文件名 | pos: x,y"
    );
  }
  if (field === "target_selector") {
    return (
      getDynamicPlaceholder("target_selector", "target_selector_type") ||
      "image: 文件名"
    );
  }
  if (field === "ocr_selector") {
    return (
      getDynamicPlaceholder("ocr_selector", "ocr_selector_type") ||
      "region: x1,y1,x2,y2 | pos: x,y"
    );
  }
  ...
  const placeholders = {
    ...
    package_name: "例如 com.example.demo，可留空",
    activity: "例如 LoginActivity、MainActivity，支持包含/精确/正则匹配",
    note: "备注",
    image_scope:
      "图片目录名（位于 Template/ 下），默认 common 或输入自定义目录名",
    expected_image_scope:
      "图片目录名（位于 Template/ 下），默认 common 或输入自定义目录名",
    fallback_image_scope:
      "图片目录名（位于 Template/ 下），默认 common 或输入自定义目录名",
  ...
  if (
    ...
    if (field === "expected") {
      return "可填 Activity 关键字；也可以用下面的期望Activity字段";
    }
  }
...
const getFieldType = (field) => {
  ...
  const properties = activeComponentDef.value.schema.properties || {};
  return properties[field] && properties[field].type
...
const getFieldOrder = (fields) => {
  ...
  if (
    currentStep &&
    (currentStep.type === "image_exists_click" ||
      currentStep.type === "image_exists_click_chain")
  ) {
    const timeoutIndex = ordered.indexOf("timeout");
    const imageFields = ["image_threshold"];
...
const saveScene = async () => {
  ...
  try {
    ...
    const cleanedSteps = JSON.parse(JSON.stringify(scenarioSteps.value)).map(
      (step) => {
...
const openElementSelector = (target = "") => {
  ...
  if (!elementSearchKeyword.value.trim()) {
    elementSearchKeyword.value = "semantic_v2";
  }
  if (!elementFilterType.value) {
    elementFilterType.value = "selector";
  }
...
const handleElementFilterChange = () => {
  elementCurrentPage.value = 1;
  loadElementsForSelector();
};
...
const loadElementsForSelector = async () => {
  ...
  try {
    ...
    if (elementFilterType.value) {
      params.element_type = elementFilterType.value;
    }
    if (elementSearchKeyword.value && elementSearchKeyword.value.trim()) {
      params.search = elementSearchKeyword.value.trim();
    }
  ...
  if (element.element_type === "image") {
    if (element.config && element.config.image_path) {
      ...
      if (target === "expected") {
        ...
        if (
          element.config.image_category &&
          config.expected_image_scope !== undefined
        ) {
      ...
      ElMessage.success(
        `已关联 ${getTargetTitle(target)}: ${element.name} (图片)`,
      );
    } else {
    ...
    if (
      element.config &&
      element.config.x !== undefined &&
      element.config.y !== undefined
    ) {
      ...
      ElMessage.success(
        `已关联 ${getTargetTitle(target)}: ${element.name} (坐标)`,
      );
    }
    ...
    if (
      ...
      ElMessage.success(
        `已关联 ${getTargetTitle(target)}: ${element.name} (区域)`,
      );
    }
  } else if (element.element_type === "selector") {
    const resourceId = element.config?.resource_id || "";
    const locatorKey = element.config?.locator_key || "";
    const boundsValue = parseBoundsToRegionValue(element.config?.bounds);
    const fallbackValue = resourceId || locatorKey || element.name;

    if (target === "ocr" && boundsValue) {
      config[typeField] = "region";
      config[valueField] = boundsValue;
    } else {
      config[typeField] = "selector";
      config[valueField] = fallbackValue;
    }

    if (target === "expected" && config.assert_type !== undefined) {
      config.assert_type = "exists";
    }

    ElMessage.success(
      `已关联${getTargetTitle(target)}: ${element.name} (定位)`,
    );
  } else {
    ElMessage.warning(
      `元素类型 ${element.element_type} 不适用于 ${getTargetTitle(target)}`,
    );
  }
};

const parseBoundsToRegionValue = (bounds) => {
  const text = String(bounds || "").trim();
...
const applyElement = (element) => {
  ...
  const targetStep =
    customDialogVisible.value && editingActiveStep.value
