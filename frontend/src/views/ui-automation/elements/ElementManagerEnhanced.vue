  ...
  <div class="element-manager">
        ...
        <div v-else class="element-detail">
          ...
          <div class="element-header">
            <div class="element-info">
              <el-form
                ref="elementHeaderFormRef"
                :model="selectedElement"
                :rules="elementHeaderRules"
                inline
              >
                <el-form-item
                  prop="name"
                ...
                >
                  <el-input
                    ...
                    :placeholder="
                      $t('uiAutomation.element.elementNamePlaceholder')
                    "
                    style="width: 300px"
    ...
    >
      ...
      <li
        v-if="
          rightClickedNode &&
          rightClickedNode.type === 'page' &&
          rightClickedNode.id !== 'unassigned'
        "
        @click="addSubPage"
      >
      ...
      <!-- "未关联页面"节点不显示编辑和删除选项 -->
      <li
        v-if="rightClickedNode && rightClickedNode.id !== 'unassigned'"
        @click="editNode"
      >
...
const elementRules = computed(() => ({
  ...
  locator_value: [
    ...
    },
    {
      min: 1,
      max: 500,
      message: t("uiAutomation.element.rules.locatorLength"),
      trigger: "blur",
    },
  ],
}));
...
const loadElementTree = async () => {
  ...
  try {
    ...
    if (unassignedElements.length > 0) {
      ...
      console.log(
        `已添加 ${unassignedElements.length} 个未关联元素到"未关联页面"节点`,
      );
      // 默认展开未关联页面节点
