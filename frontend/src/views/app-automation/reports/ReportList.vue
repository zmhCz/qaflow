<template>
  <div class="report-list">
      ...
      <el-tab-pane label="套件报告" name="suite">
        ...
        <div class="filters">
          <el-row :gutter="20">
            <el-col :span="4">
              <el-select
                v-model="suiteProjectFilter"
                placeholder="全部项目"
                clearable
                filterable
                @change="loadSuiteReports"
              >
                <el-option
            ...
            <el-col :span="6">
              <el-input
                ...
                placeholder="搜索套件名称"
                clearable
                @clear="loadSuiteReports"
                @keyup.enter="loadSuiteReports"
              >
                <template #prefix
            ...
            <el-col :span="6">
              <el-button type="primary" @click="loadSuiteReports"
            ...
            <template #default="{ row }">
              <div class="step-stats">
                <span style="color: #67c23a"
                  >通过 {{ row.passed_count || 0 }}</span
                >
                <el-divider direction="vertical" />
                <span style="color: #f56c6c"
                  >失败 {{ row.failed_count || 0 }}</span
                >
                <el-divider direction="vertical" />
          ...
          <el-table-column label="操作" min-width="200">
            <template #default="{ row }">
              ...
              <el-button
                ...
                link
                size="small"
                @click="viewSuiteAllureReport(row)"
                >Allure报告</el-button
              >
              <el-button
                type="danger"
      ...
      <el-tab-pane label="用例报告" name="case">
        ...
        <div class="filters">
          <el-row :gutter="20">
            <el-col :span="4">
              <el-select
                v-model="caseProjectFilter"
                placeholder="全部项目"
                clearable
                filterable
                @change="loadCaseReports"
              >
                <el-option
                  v-for="p in projectList"
                  :key="p.id"
            ...
            <el-col :span="6">
              <el-input
                ...
                clearable
                @clear="loadCaseReports"
                @keyup.enter="loadCaseReports"
              >
                <template #prefix
                  ><el-icon><Search /></el-icon
                ></template>
            ...
            <el-col :span="6">
              <el-button type="primary" @click="loadCaseReports"
                ><el-icon><Search /></el-icon>查询</el-button
              >
              <el-button
                @click="
    ...
    <el-dialog v-model="suiteDetailVisible" title="套件报告详情" width="750px">
      <div v-if="selectedSuite">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="创建人">{{
            selectedSuite.created_by_name || "-"
          }}</el-descriptions-item>
          <el-descriptions-item label="最后执行">{{
            formatDateTime(selectedSuite.last_run_at)
          }}</el-descriptions-item>
        </el-descriptions>
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="8">
              <div class="detail-stat info-bg">
                <div class="detail-stat-num">
                  {{ selectedSuite.test_case_count || 0 }}
                </div>
      ...
      <template #footer>
        ...
        <el-button
          type="primary"
          @click="
            suiteDetailVisible = false;
            viewSuiteExecutions(selectedSuite);
          "
          >查看执行记录</el-button
        >
      </template>
    ...
    <!-- ==================== 套件执行记录弹窗 ==================== -->
    <el-dialog
      v-model="suiteExecVisible"
      :title="`执行记录 - ${selectedSuite?.name || ''}`"
      width="900px"
    >
      <el-table
        :data="suiteExecRecords"
        v-loading="suiteExecLoading"
        border
        stripe
        max-height="500"
      >
...
import StandardExecutionReportDialog from "./components/StandardExecutionReportDialog.vue";
import {
  getExecutionList,
  deleteExecution,
  getTestSuiteList,
...
import {
  getDisplayStatus,
  formatDateTime,
} from "@/utils/app-automation-helpers.js";
...
const suiteStatsCards = computed(() => {
  ...
  const avgRate =
    executed.length > 0
      ? Math.round(
          executed.reduce((sum, s) => sum + getSuitePassRate(s), 0) /
            executed.length,
        )
      : 0;
  return [
...
async function viewSuiteAllureReport(suite) {
  ...
  try {
    ...
    if (withReport) {
      window.open(
        `/api/app-automation/executions/${withReport.id}/report/`,
        "_blank",
      );
    } else {
...
const caseReportSummary = ref(null);
...
const caseStatsCards = computed(() => {
  ...
  const avgRate =
    data.length > 0
      ? Math.round(
          data.reduce((sum, r) => sum + (r.pass_rate || 0), 0) / data.length,
        )
      : 0;
  return [
...
async function viewCaseDetail(row) {
  selectedCase.value = row;
  caseDetailVisible.value = true;
  caseReportSummary.value = null;
  caseSummaryLoading.value = true;
  try {
    const res = await getExecutionReportSummary(row.id);
    caseReportSummary.value = res.data?.data || null;
  } catch {
    ElMessage.warning("标准报告摘要加载失败，已显示基础执行信息");
  } finally {
    caseSummaryLoading.value = false;
  }
}
...
async function deleteCaseReport(row) {
  try {
    await ElMessageBox.confirm("确认删除该执行报告？", "删除确认", {
      type: "warning",
    });
