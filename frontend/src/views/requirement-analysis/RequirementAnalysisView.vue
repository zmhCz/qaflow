      ...
      <div v-if="isGenerating || showResults" class="generation-progress">
        <div class="progress-card">
          ...
          <div v-if="showResults" class="completion-actions">
            ...
            <button class="xmind-btn" @click="downloadXmindTestCases">
...
import { downloadXmind, worksheetRowsToTestCases } from "@/utils/xmindExporter";
...
export default {
  ...
  methods: {
    ...
    async downloadXmindTestCases() {
      try {
        const worksheetData = this.buildWorksheetDataFromGenerationResult();
        const testCases = worksheetRowsToTestCases(worksheetData);
        ...
        const taskId = this.generationResult.task_id;
        const fileName = this.$t("requirementAnalysis.xmindFileName", {
          taskId,
          date: new Date().toISOString().slice(0, 10),
        });

        downloadXmind(testCases, {
          fileName,
          rootTitle:
            this.manualInput.title ||
            this.uploadForm.title ||
            this.$t("requirementAnalysis.xmindRootTitle"),
          preconditionLabel: this.$t("requirementAnalysis.excelPrecondition"),
          stepsLabel: this.$t("requirementAnalysis.excelTestSteps"),
          expectedLabel: this.$t("requirementAnalysis.excelExpectedResult"),
          priorityLabel: this.$t("requirementAnalysis.excelPriority"),
          untitledCaseLabel: this.$t("requirementAnalysis.untitledCase"),
        });

        ElMessage.success(this.$t("requirementAnalysis.xmindDownloadSuccess"));
      } catch (error) {
        console.error(
          this.$t("requirementAnalysis.xmindDownloadFailed"),
          error,
        );
        ElMessage.error(
          this.$t("requirementAnalysis.xmindDownloadFailed") +
    ...
    buildWorksheetDataFromGenerationResult() {
      const finalTestCases = this.generationResult.final_test_cases;
      const filteredContent = this.filterTestCasesOnly(finalTestCases);
      const tableFormat = this.parseTableFormat(filteredContent);
      let worksheetData = [];
...
.completion-actions .xmind-btn {
  background: #0f766e;
  color: white;
  font-size: 1rem;
}

.completion-actions .xmind-btn:hover {
  background: #0b5f59;
  transform: translateY(-2px);
