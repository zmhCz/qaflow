<template>
  <div class="environment-management">
    ...
    <el-dialog
      ...
      :title="
        editingEnvironment
          ? $t('apiTesting.environment.editEnvironment')
          : $t('apiTesting.environment.createEnvironment')
      "
      width="800px"
    ...
    >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="120px">
        <el-form-item
          :label="$t('apiTesting.environment.environmentName')"
          prop="name"
        >
          <el-input
            v-model="form.name"
            :placeholder="$t('apiTesting.environment.inputEnvironmentName')"
          />
        </el-form-item>
        ...
        <el-form-item :label="$t('apiTesting.environment.scope')" prop="scope">
          <el-radio-group v-model="form.scope" @change="onScopeChange">
            <el-radio value="GLOBAL">{{
              $t("apiTesting.environment.scopeTypes.global")
            }}</el-radio>
            <el-radio value="LOCAL">{{
              $t("apiTesting.environment.scopeTypes.local")
            }}</el-radio>
          </el-radio-group>
        ...
        >
          <el-select
            ...
            :placeholder="$t('apiTesting.environment.selectRelatedProject')"
          >
            <el-option
              v-for="project in projects"
              :key="project.id"
        ...
        <el-form-item
          :label="$t('apiTesting.environment.environmentVariables')"
          prop="variables"
        >
          <div class="variables-editor">
            <div class="variables-header">
              <div class="column">
                {{ $t("apiTesting.environment.variableName") }}
              </div>
              <div class="column">
                {{ $t("apiTesting.environment.initialValue") }}
              </div>
              <div class="column">
                {{ $t("apiTesting.environment.currentValue") }}
              </div>
    ...
    >
      <div v-if="viewingEnvironment" class="view-variables">
        <el-table :data="viewVariables" style="width: 100%">
          <el-table-column
            prop="key"
            :label="$t('apiTesting.environment.variableName')"
            width="150"
          />
          <el-table-column
            prop="initialValue"
            :label="$t('apiTesting.environment.initialValue')"
          />
          <el-table-column
            prop="currentValue"
            :label="$t('apiTesting.environment.currentValue')"
          />
        </el-table>
      </div>
...
const rules = computed(() => ({
  name: [
    {
      required: true,
      message: t("apiTesting.environment.inputEnvironmentName"),
      trigger: "blur",
    },
  ],
  scope: [
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingEnvironment.value) {
      await api.put(
        `/api-testing/environments/${editingEnvironment.value.id}/`,
        data,
      );
