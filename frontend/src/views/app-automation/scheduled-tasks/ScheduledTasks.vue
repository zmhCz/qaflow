  ...
  <div class="scheduled-tasks">
    ...
    <el-table :data="tasks" v-loading="loading" border>
      ...
      <el-table-column label="通知" width="100">
        <template #default="{ row }">
          <el-tag
            v-if="row.notification_type"
            :type="
              row.notification_type === 'webhook'
                ? 'primary'
      ...
      <el-table-column label="操作" width="200" fixed="right">
        <template #default="{ row }">
          <el-button size="small" @click="runNow(row)" :loading="row._running"
            >执行</el-button
          >
    ...
    >
      <el-form :model="form" label-width="110px">
        ...
        <el-form-item
          v-if="form.task_type === 'TEST_SUITE'"
          label="测试套件"
          required
        >
          <el-select
        ...
        <el-form-item
          v-if="form.notify_on_success || form.notify_on_failure"
          label="通知类型"
        >
          <el-select
        ...
        <el-form-item
          v-if="
            (form.notify_on_success || form.notify_on_failure) &&
...
const loadTasks = async () => {
  ...
  try {
    ...
    tasks.value = (res.data.results || []).map((t) => ({
      ...t,
      _running: false,
    }));
...
const resetForm = () =>
  Object.assign(form, { ...defaultForm, notify_emails: [] });
const resetFilters = () => {
...
const submitForm = async () => {
  ...
  try {
    ...
    ElMessage.error(
      e.response?.data?.detail || e.response?.data?.message || "操作失败",
    );
  } finally {
