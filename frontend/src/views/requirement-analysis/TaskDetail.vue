  ...
  <div class="task-detail">
    <div class="page-header">
      ...
      <div class="header-actions">
        ...
        <button
          v-if="testCases.length > 0"
          class="export-btn xmind-export-btn"
          @click="exportToXmind"
          :disabled="isExporting"
        >
          {{ $t("taskDetail.exportXmindBtn") }}
        </button>
      </div>
    </div>
...
import { downloadXmind } from "@/utils/xmindExporter";
...
export default {
  ...
  methods: {
    ...
    exportToXmind() {
      ...
      try {
        const dateStr = new Date().toISOString().slice(0, 10);
        const fileName = this.$t("taskDetail.xmindFileName", {
          taskId: this.taskId,
          date: dateStr,
        });
        downloadXmind(this.testCases, {
          fileName,
          rootTitle: this.task.title || this.$t("taskDetail.xmindRootTitle"),
          preconditionLabel: this.$t("taskDetail.tablePrecondition"),
          stepsLabel: this.$t("taskDetail.tableSteps"),
          expectedLabel: this.$t("taskDetail.tableExpected"),
          priorityLabel: this.$t("taskDetail.tablePriority"),
          untitledCaseLabel: this.$t("taskDetail.untitledCase"),
        });
...
.xmind-export-btn:hover:not(:disabled) {
  background: #0b5f59;
}
