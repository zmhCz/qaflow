      ...
      <el-table :data="projects" v-loading="loading" border stripe>
        ...
        >
          <template #default="{ row }">
            <span v-if="row.android_app_package_code"
              >{{ row.android_app_package_name || "应用" }}（{{
                row.android_app_package_code
              }}）</span
            >
            <span v-else>-</span>
          </template>
        ...
        <el-table-column
          ...
          show-overflow-tooltip
        >
          <template #default="{ row }">{{ row.ios_bundle_id || "-" }}</template>
        </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item label="Android包名">
          <el-select
            v-model="form.android_app_package"
            placeholder="请选择 Android APP 包名"
            clearable
            filterable
            style="width: 100%"
          >
        ...
        <el-form-item label="iOS Bundle ID">
          <el-input
            v-model.trim="form.ios_bundle_id"
            placeholder="例如：com.company.app.ios"
            clearable
          />
        </el-form-item>
    ...
    <el-dialog v-model="detailVisible" title="项目详情" width="600px">
      <div v-if="selectedProject">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="Android包名" :span="2">
            {{
              selectedProject.android_app_package_code
                ? `${selectedProject.android_app_package_name || "应用"}（${selectedProject.android_app_package_code}）`
                : "未设置"
            }}
          </el-descriptions-item>
          <el-descriptions-item label="iOS Bundle ID" :span="2">
            {{ selectedProject.ios_bundle_id || "未设置" }}
          </el-descriptions-item>
          <el-descriptions-item label="负责人">{{
            selectedProject.owner_name || "-"
          }}</el-descriptions-item>
          <el-descriptions-item label="成员数"
            >{{ selectedProject.member_count || 0 }} 人</el-descriptions-item
          >
          <el-descriptions-item label="测试用例"
            >{{ selectedProject.test_case_count || 0 }} 个</el-descriptions-item
          >
          <el-descriptions-item label="测试套件"
            >{{
              selectedProject.test_suite_count || 0
            }}
            个</el-descriptions-item
          >
          <el-descriptions-item label="开始日期">{{
            selectedProject.start_date || "未设置"
          }}</el-descriptions-item>
          <el-descriptions-item label="结束日期">{{
            selectedProject.end_date || "未设置"
          }}</el-descriptions-item>
...
async function loadPackages() {
  try {
    ...
    appPackages.value = res.data.results || res.data || [];
  } catch {
    appPackages.value = [];
  }
}
...
function openEditDialog(row) {
  ...
  Object.assign(form, {
    ...
    status: row.status,
    android_app_package: row.android_app_package || null,
