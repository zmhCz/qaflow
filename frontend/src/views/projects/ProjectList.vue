    ...
    >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item :label="$t('project.status')" prop="status">
          <el-select
            v-model="form.status"
            :placeholder="$t('project.selectStatus')"
          >
            <el-option :label="$t('project.active')" value="active" />
            <el-option :label="$t('project.paused')" value="paused" />
            <el-option :label="$t('project.completed')" value="completed" />
...
const rules = {
  name: [
    {
      required: true,
      message: computed(() => t("project.projectNameRequired")),
      trigger: "blur",
    },
    {
      min: 2,
      max: 200,
      message: computed(() => t("project.projectNameLength")),
      trigger: "blur",
    },
  ],
  status: [
    {
      required: true,
      message: computed(() => t("project.projectStatusRequired")),
      trigger: "change",
    },
  ],
};
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        ElMessage.error(
          isEdit.value ? t("project.updateFailed") : t("project.createFailed"),
        );
      } finally {
